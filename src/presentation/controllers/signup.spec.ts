...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 if password and its confirmation are different', () => {
    const { sut } = makeSut();
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should return 500 if AddAccount throws an exception', () => {
    const { sut, addAccountStub } = makeSut();
    jest.spyOn(addAccountStub, 'add').mockImplementationOnce((addAccountModel: AddAccountModel) => {
      throw new Error();
    });
    const httpRequest = {
      body: {
  ...
  test('Should return 200 if all data is correct', () => {
    const { sut } = makeSut();
    const httpRequest = {
      body: {
        name: 'valid_name',
